...
let render tmppath finalpath outtex md5 backcolor resolution_in_dpi =
    ...
    let f = (Util.open_out_unless_exists (tmpprefix ^ ".tex")) in
      begin
        ...
        ) else if (Sys.command (cmd_dvipng tmpprefix (finalpath^"/"^md5^".png") backcolor resolution_in_dpi) != 0)
          then (
