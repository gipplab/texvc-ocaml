...
(* *)
let render tmppath finalpath tree backcolor resolution_in_dpi =
    ...
    Render.render tmppath finalpath outtex md5 backcolor resolution_in_dpi
    end
...
let _ =
    ...
    let resolution_in_dpi = (try int_of_string Sys.argv.(6) with _ -> 120) in
        try render Sys.argv.(1) Sys.argv.(2) (
            Parser.tex_expr lexer_token_safe (
                Lexing.from_string Sys.argv.(3))
             ) (try Sys.argv.(5) with _ -> "rgb 1.0 1.0 1.0") resolution_in_dpi
